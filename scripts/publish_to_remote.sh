...
PUBLISH_DIR="${PUBLISH_DIR:-$ROOT_DIR/.artifacts/publish/api}"
REMOTE_PORT="${REMOTE_PORT:-22}"
REMOTE_DOCKERFILE_NAME="${REMOTE_DOCKERFILE_NAME:-Dockerfile}"
REMOTE_IMAGE_NAME="${REMOTE_IMAGE_NAME:-proxytransfer-api:latest}"
REMOTE_CONTAINER_NAME="${REMOTE_CONTAINER_NAME:-proxytransfer-api}"
REMOTE_RUNTIME_ENV_PATH="${REMOTE_RUNTIME_ENV_PATH:-}"
...
check_remote_port_conflicts() {
	...
	command -v ss >/dev/null 2>&1 || {
		printf '[publish] 远端缺少 ss，跳过端口占用预检。\n' >&2
		return 0
	}
	...
	if [[ "\${#conflicts[@]}" -gt 0 ]]; then
		printf '[publish] 远端端口冲突，容器未启动。请调整 CONTAINER_PORTS 或释放这些端口：\n' >&2
		printf '  - %s\n' "\${conflicts[@]}" >&2
		return 1
	fi
}
