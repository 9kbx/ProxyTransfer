...
cp "$SCRIPT_DIR/restart_web.sh" "$PUBLISH_DIR/"
...
load_env_file() {
	...
	while IFS= read -r line || [[ -n "\$line" ]]; do
		...
		[[ "\$line" =~ ^[[:space:]]*# ]] && continue
		[[ "\$line" == *=* ]] || {
			printf '[publish-web] 远端运行时配置无效: %s\n' "\$line" >&2
			return 1
		}
		...
		[[ "\$key" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] || {
			printf '[publish-web] 远端运行时配置键无效: %s\n' "\$key" >&2
			return 1
		}
		...
		if [[ \${#value} -ge 2 ]]; then
			...
			if [[ "\$quote_char" == '"' || "\$quote_char" == "'" ]]; then
				if [[ "\${value: -1}" != "\$quote_char" ]]; then
					printf '[publish-web] 远端运行时配置引号不匹配: %s\n' "\$key" >&2
					return 1
				fi
				value="\${value:1:\${#value}-2}"
			fi
...
check_remote_port_conflicts() {
	...
	command -v ss >/dev/null 2>&1 || {
		printf '[publish-web] 远端缺少 ss，跳过端口占用预检。\n' >&2
		return 0
	}
	...
	if [[ "\${#conflicts[@]}" -gt 0 ]]; then
		printf '[publish-web] 远端端口冲突，容器未启动。请调整 CONTAINER_PORTS 或释放这些端口：\n' >&2
		printf '  - %s\n' "\${conflicts[@]}" >&2
		return 1
	fi
}
...
if [[ -n "\$REMOTE_RUNTIME_ENV_PATH_VALUE" ]]; then
	[[ -f "\$REMOTE_RUNTIME_ENV_PATH_VALUE" ]] || {
		printf '[publish-web] 未找到远端运行时配置文件: %s\n' "\$REMOTE_RUNTIME_ENV_PATH_VALUE" >&2
		exit 1
	}
