...
load_env_file() {
	...
	while IFS= read -r line || [[ -n "\$line" ]]; do
		...
		[[ "\$line" =~ ^[[:space:]]*# ]] && continue
		[[ "\$line" == *=* ]] || {
			printf '[restart-web] 远端运行时配置无效: %s\n' "\$line" >&2
			return 1
		}
		...
		[[ "\$key" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] || {
			printf '[restart-web] 远端运行时配置键无效: %s\n' "\$key" >&2
			return 1
		}
		...
		if [[ \${#value} -ge 2 ]]; then
			...
			if [[ "\$quote_char" == '"' || "\$quote_char" == "'" ]]; then
				if [[ "\${value: -1}" != "\$quote_char" ]]; then
					printf '[restart-web] 远端运行时配置引号不匹配: %s\n' "\$key" >&2
					return 1
				fi
				value="\${value:1:\${#value}-2}"
			fi
...
check_remote_port_conflicts() {
	...
	command -v ss >/dev/null 2>&1 || {
		printf '[restart-web] 远端缺少 ss，跳过端口占用预检。\n' >&2
		return 0
	}
	...
	if [[ "\${#conflicts[@]}" -gt 0 ]]; then
		printf '[restart-web] 远端端口冲突，容器未启动。请调整 CONTAINER_PORTS 或释放这些端口：\n' >&2
		printf '  - %s\n' "\${conflicts[@]}" >&2
		return 1
	fi
}
...
if [[ -n "\$REMOTE_RUNTIME_ENV_PATH_VALUE" ]]; then
	[[ -f "\$REMOTE_RUNTIME_ENV_PATH_VALUE" ]] || {
		printf '[restart-web] 未找到远端运行时配置文件: %s\n' "\$REMOTE_RUNTIME_ENV_PATH_VALUE" >&2
		exit 1
	}
...
API_PROJECT_PATH="${API_PROJECT_PATH:-$ROOT_DIR/ProxyTransfer.Api/ProxyTransfer.Api.csproj}"
API_PROJECT_DIR="$(cd -- "$(dirname -- "$API_PROJECT_PATH")" && pwd)"
API_DOCKERFILE_PATH="${API_DOCKERFILE_PATH:-$API_PROJECT_DIR/Dockerfile}"
WEB_PROJECT_DIR="${WEB_PROJECT_DIR:-$ROOT_DIR/ProxyTransfer.Web}"
WEB_DIST_DIR="${WEB_DIST_DIR:-$WEB_PROJECT_DIR/dist}"
PUBLISH_CONFIGURATION="${PUBLISH_CONFIGURATION:-Release}"
PUBLISH_DIR="${PUBLISH_DIR:-$ROOT_DIR/.artifacts/publish/web}"
REMOTE_PORT="${REMOTE_PORT:-22}"
REMOTE_DOCKERFILE_NAME="${REMOTE_DOCKERFILE_NAME:-Dockerfile}"
REMOTE_IMAGE_NAME="${WEB_REMOTE_IMAGE_NAME:-${REMOTE_IMAGE_NAME:-proxytransfer-web:latest}}"
REMOTE_CONTAINER_NAME="${WEB_REMOTE_CONTAINER_NAME:-${REMOTE_CONTAINER_NAME:-proxytransfer-web}}"
REMOTE_PATH="${WEB_REMOTE_PATH:-${REMOTE_PATH:-/apps/proxytransfer/web}}"
REMOTE_RUNTIME_ENV_PATH="${WEB_REMOTE_RUNTIME_ENV_PATH:-${REMOTE_RUNTIME_ENV_PATH:-/apps/proxytransfer/config/web.runtime.env}}"
...
REMOTE_RESTART_SCRIPT_NAME="restart_web.sh"
REMOTE_RESTART_SCRIPT_PATH="${REMOTE_PATH%/}/$REMOTE_RESTART_SCRIPT_NAME"

[[ -f "$API_DOCKERFILE_PATH" ]] || fail "未找到 Dockerfile: $API_DOCKERFILE_PATH"
[[ -f "$WEB_PROJECT_DIR/package.json" ]] || fail "未找到前端项目: $WEB_PROJECT_DIR"
...
log "构建前端"
npm --prefix "$WEB_PROJECT_DIR" run build

[[ -d "$WEB_DIST_DIR" ]] || fail "前端构建产物不存在: $WEB_DIST_DIR"

log "发布 API"
rm -rf "$PUBLISH_DIR"
dotnet publish "$API_PROJECT_PATH" -c "$PUBLISH_CONFIGURATION" -o "$PUBLISH_DIR"

log "复制前端到 API 发布目录"
rm -rf "$PUBLISH_DIR/wwwroot"
mkdir -p "$PUBLISH_DIR/wwwroot"
cp -R "$WEB_DIST_DIR"/. "$PUBLISH_DIR/wwwroot/"
cp "$API_DOCKERFILE_PATH" "$PUBLISH_DIR/$REMOTE_DOCKERFILE_NAME"
