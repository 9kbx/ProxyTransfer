#!/usr/bin/env bash
# restart_web.sh — 放在远端服务器上直接执行，重启 Web/API 容器。
# 用法:
#   ./restart_web.sh
#   ./restart_web.sh /path/to/web.runtime.env
#
# 默认读取 /apps/proxytransfer/config/web.runtime.env，
# Docker 构建上下文默认为本脚本所在目录。

set -Eeuo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
RUNTIME_ENV_FILE="${1:-/apps/proxytransfer/config/web.runtime.env}"
...
#   WEB_REMOTE_IMAGE_NAME / REMOTE_IMAGE_NAME
#   WEB_REMOTE_CONTAINER_NAME / REMOTE_CONTAINER_NAME
...
IMAGE_NAME="${WEB_REMOTE_IMAGE_NAME:-${REMOTE_IMAGE_NAME:-proxytransfer-web:latest}}"
CONTAINER_NAME="${WEB_REMOTE_CONTAINER_NAME:-${REMOTE_CONTAINER_NAME:-proxytransfer-web}}"
